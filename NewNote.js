...
export function NewNoteUi({ navigation }) {
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.formArea}>
        ...
        <View>
          <Pressable onPress={saveNote} style={styles.btnPrimary}>
            <Text style={styles.btnText}>{"Save Note"}</Text>
          </Pressable>
        </View>

        <View>
          <Pressable onPress={noteList}  style={styles.btnAsh}>
            <Text style={styles.btnAshText}>{"View Note List"}</Text>
          </Pressable>
        </View>
      </View>
    </SafeAreaView>
  ...
  async function saveNote() {
    ...
    fetch("http://10.0.2.2/MyNotes/save-note.php", {
      method: "POST",
      ...
      .then((text) => {
        if (text == "Success") {
          ...
          Alert.alert("Message", "Successfully Added..");
